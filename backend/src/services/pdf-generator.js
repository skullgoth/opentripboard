...
export async function generateTripPDF(trip, activities, tripBuddies = [], options = {}) {
  ...
  return new Promise((resolve, reject) => {
    try {
      // bufferPages: true keeps all pages in memory so we can add footers with
      // correct "Page X of Y" after the entire document is rendered (total page
      // count isn't known upfront). Pages are flushed just before doc.end().
      ...
      // Two-pass footer rendering: now that all content is rendered and the total
      // page count is known, revisit each buffered page to stamp the footer.
      // save()/restore() isolates font/color changes so they don't bleed into
      // any remaining document state when switching between pages.
      ...
      // Flush all buffered pages to the output stream before ending
      doc.flushPages();
      doc.end();
    } catch (error) {
      reject(error);
...
function renderActivities(doc, trip, activities) {
  ...
  dates.forEach((date, index) => {
    // Page break threshold: 680px leaves ~110px for a day header + at least one
    // activity before hitting the bottom margin (A4 = 792pt usable height).
    if (doc.y > 680) {
      doc.addPage();
    }
...
function renderActivity(doc, activity) {
  // 700px threshold is higher than the day-header threshold (680px) because
  // individual activities are smaller than a full day section
  if (doc.y > 700) {
    doc.addPage();
  }
  ...
  if (activity.location || activity.address) {
    doc
      ...
      .font('Helvetica')
      .fillColor('#4a4a6a')
      .text(`📍 ${activity.address || activity.location}`, startX + 15);
  }
  ...
  if (activity.metadata?.isReservation) {
    ...
    if (meta.confirmationCode) {
      doc
        ...
        .font('Helvetica')
        .fillColor('#059669')
        .text(`✓ Confirmation: ${meta.confirmationCode}`, startX + 15);
    }
...
function renderBudgetPage(doc, trip, expenses, summary) {
  ...
  if (summary?.byCategory && Array.isArray(summary.byCategory) && summary.byCategory.length > 0) {
    ...
    summary.byCategory.forEach((item) => {
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor('#4a4a6a')
        .text(`• ${item.category}: ${currency} ${amount.toLocaleString()}`, 70);
    });
  ...
  if (expenses && expenses.length > 0) {
    ...
    // Cap at 30 expenses to keep the PDF reasonable in size; additional expenses
    // are indicated with a "... and N more" note below the table
    const currency = trip.currency || 'USD';
    expenses.slice(0, 30).forEach((expense) => {
      // 720px threshold is tight since expense rows are compact (single line each)
      if (doc.y > 720) {
        doc.addPage();
      }
...
function groupActivitiesByDate(activities) {
  // Group activities into day buckets keyed by 'yyyy-MM-dd', then chronologically
  // sort within each day. Activities without a start_time are excluded since they
  // can't be placed on the itinerary timeline.
  // Note: re-sorting on every insert is O(n^2 log n) worst case but n (activities
  // per day) is small enough that this is negligible.
